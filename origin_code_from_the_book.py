...
def get_adj_matrix(file_name):
    ...
    with open(file_name, 'r') as f:
        for line in f.readlines():
            temp = [int(char) for char in line.strip().split()]
            matrix.append(temp)
    return matrix

attraction_distances = get_adj_matrix('attractions_' + str(ATTRACTION_COUNT) + '.csv')

# with open('attractions_' + str(ATTRACTION_COUNT) + '.csv') as file:
